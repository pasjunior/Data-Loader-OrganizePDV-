...
cur.execute("SELECT DISTINCT UNIDADE FROM PRODUTO")

rows = cur.fetchall()
print(rows)


# Verificar se o arquivo 'dePara_unidade_ServSoft.xlsx' existe

if not os.path.exists('dePara_unidade_ServSoft.xlsx'):
    # Se o arquivo não existir, criar um novo arquivo com uma planilha 'UNIDADE'
    dePara = pd.DataFrame({'de': [], 'para': []})
    writer = pd.ExcelWriter('dePara_unidade_ServSoft.xlsx', engine='xlsxwriter')
    ...
    dePara = pd.read_excel('dePara_unidade_ServSoft.xlsx', sheet_name='UNIDADE')
...
dePara.to_excel('dePara_unidade_ServSoft.xlsx', sheet_name='UNIDADE', index=False)
...
cur.execute("SELECT PRODUTO_ID, DESCRICAO, COD_BARRA, QUANT_ESTOQUE, UNIDADE, NCM FROM PRODUTO") # Conectando com o banco de dados a ser migrado
rows = cur.fetchall()
...
for row in rows:
# UND_VENDA
    und_venda = 'EM BRANCO' if row[4] is None else row[4]
    if (dePara['de'] == und_venda).any():
        sigla_und_venda = dePara.loc[dePara['de'] == und_venda, 'para'].iloc[0]
    else:
        dfErros.append({'codigo': row[0], 'descrição': row[1], 'divergencia': f'Unidade de venda {und_venda} não existe no dePara'})
        continue
    ...
    if not unidade_venda:
        dfErros.append({'codigo': row[0], 'descrição': row[1], 'divergencia': f'Unidade de venda {sigla_und_venda} não existe no PDV'})
        continue
...
# UND_COMPRA
    ...
    if (dePara['de'] == und_compra).any():
        ...
        dfErros.append({'codigo': row[0], 'descrição': row[1], 'divergencia': f'Unidade de Compra {und_compra} não existe no dePara'})
        continue
    ...
    if not unidade_compra:
        dfErros.append({'codigo': row[0], 'descrição': row[1], 'divergencia': f'Unidade de Compra {sigla_und_compra} não existe no PDV'})
        continue
...
# NCM
    ...
    for ncmpdv in itemsJson_ncm:
        if ncmpdv["CNCM"] == cod_ncm:
            ncm = ncmpdv
    if not ncm:
        dfErros.append({'codigo': row[0], 'descrição': row[1], 'divergencia': f'Código NCM {cod_ncm} não existe no PDV'})
        continue
    ...
    payload = {
        ...
        "ONCM-NID": ncm['NID'],
        "CTIPO": row[0], # não tem na base
        "OTE-NID": 1,
        "OTS-NID": 2,
        "NPLIQ": float(row[3]), # Tive que converter em float pq a biblioteca requests n reconhece números decimais
        "NPBRU": float(row[3])
}
...
if len(dfErros) > 0:
    ...
    dfErros.to_excel('erros_ServSoft.xlsx', sheet_name='ERROS', index=False)
    messagebox.showinfo("Atenção", f'Tratar os erros em: erros_ServSoft.xlsx')
